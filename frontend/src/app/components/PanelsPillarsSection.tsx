...
const PanelsPillarsSection: React.FC = () => {
  return (
    <section className="py-16 bg-gradient-to-b from-violet-50/50 via-indigo-50/40 to-violet-50/50 dark:from-violet-950/50 dark:via-indigo-950/40 dark:to-violet-950/50 text-gray-900 dark:text-white transition-colors duration-300">
      <div className="container mx-auto px-4 flex flex-col items-center">
        <h2 className="mt-14 mb-6 text-2xl md:text-3xl font-semibold tracking-wide text-gray-800 dark:text-gray-100 relative inline-block text-center">
          Our Panels & Pillars
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {panelsPillarsData.map((item, index) => (
            <div
              key={index}
              className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-sm p-6 rounded-lg border border-violet-200 dark:border-violet-500/20 hover:border-violet-400 dark:hover:border-violet-400/40 shadow-lg dark:shadow-md hover:shadow-xl transition-all"
            >
              <div className="text-violet-500 dark:text-violet-400 mb-4">{item.icon}</div>
              <h3 className="text-xl font-semibold mb-2 text-gray-900 dark:text-white">{item.name}</h3>
